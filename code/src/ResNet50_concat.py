...
logging.basicConfig(level=logging.INFO,
                    filename='ResNet50_concat_info.log',
                    filemode='a',
...
def learning_curve(record_train, record_test=None):
    ...
    plt.xlabel("epoch")
    plt.ylabel("accuracy")
    plt.savefig('ResNet50_concat.png')
    # plt.show()
...
def main():
    ...
    net = torch.load('./ResNet50_concat.pth')
    net = net.to(device)
    ...
    # torch.save(net, "ResNet50_concat.pth")
    # learning_curve(record_train, record_test)
    pred_label,true_label,pred_proba = test(net,valid_data_loader, criterion, device)
    pd.DataFrame(pred_label,columns=['label']).to_csv('../output/temp_data/validation_noattention_pred_label.csv')
    pd.DataFrame(pred_proba,columns=['proba']).to_csv('../output/temp_data/validation_noattention_pred_proba.csv')
    pd.DataFrame(true_label,columns=['label']).to_csv('../output/temp_data/validation_noattention_true_label.csv')
