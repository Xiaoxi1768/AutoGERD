...
def learning_curve(record_train, record_test=None):
    ...
    plt.xlabel("epoch")
    plt.ylabel("accuracy")
    plt.savefig('Train_Epoch_ACC_two.png')
    # plt.show()
...
def plot_AUC(train_AUC, record_test=None):
    ...
    plt.savefig('Train_Epoch_AUC_two.png')
...
def main():
    ...
    torch.save(net, "ResNet50_last.pth")
    pd.DataFrame(record_train,columns=['acc']).to_csv('../output/temp_data/record_train.csv')
    pd.DataFrame(train_AUC,columns=['auc']).to_csv('../output/temp_data/train_AUC.csv')
    # learning_curve(record_train)
    # plot_AUC(train_AUC)
    # pred_label,true_label,pred_proba = test(net,valid_data_loader, criterion, device)
    # pd.DataFrame(pred_label,columns=['label']).to_csv('../output/temp_data/validation_pred_label.csv')
    # pd.DataFrame(pred_proba,columns=['proba']).to_csv('../output/temp_data/validation_pred_proba.csv')
    # pd.DataFrame(true_label,columns=['label']).to_csv('../output/temp_data/validation_true_label.csv')
