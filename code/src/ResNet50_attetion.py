...
# train_image_paths = train_image_paths[train_image_paths['label']!=5]
# valid_image_paths = valid_image_paths[valid_image_paths['label']!=5]
...
#     logging.info("test_loss: {:.3f} | test_acc: {:6.3f}%"\
#         .format(loss.item(), test_acc))
#     print("************************************\n")
#     logging.info("************************************\n")
#     net.train()
...
def learning_curve(record_train, record_test=None):
    ...
    plt.xlabel("epoch")
    plt.ylabel("accuracy")
    plt.savefig('ResNet50_not_attetion.png')
    # plt.show()
...
def main():
    ...
    net = torch.load('./ResNet50_attetion.pth')
    net = net.to(device)
    ...
    # torch.save(net, "ResNet50_not_attetion.pth")
    # learning_curve(record_train, record_test)
    pred_label,true_label,pred_proba = test(net,valid_data_loader, criterion, device)
    pd.DataFrame(pred_label,columns=['label']).to_csv('../output/temp_data/validation_attention_pred_label.csv')
    pd.DataFrame(pred_proba,columns=['proba']).to_csv('../output/temp_data/validation_attention_pred_proba.csv')
    pd.DataFrame(true_label,columns=['label']).to_csv('../output/temp_data/validation_attention_true_label.csv')
